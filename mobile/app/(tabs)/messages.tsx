...
const MessagesScreen = () => {
  const insets = useSafeAreaInsets(); // makes content stay in safe area
  const [searchText, setSearchText] = useState(""); // keep track of search query
  const [conversationsList, setConversationsList] = useState(CONVERSATIONS); // List of conversations (just default hard coded conversations.)
  const [selectedConversation, setSelectedConversation] = useState<ConversationType | null>(null); // Can now select conversation out of list to see content of conversations
  const [isChatOpen, setIsChatOpen] = useState(false); // Use to open chat model
